...
@dp.callback_query_handler(text="earned")
async def support(call: CallbackQuery):
    stat = await select_stat()
    await call.message.edit_caption(f"<b>💰 Ваша прибыль:</b> <code>{stat[3]}.00 Rub</code>",
                                    reply_markup=earned_menu)


@dp.callback_query_handler(text_startswith="earned:")
async def earned(call: CallbackQuery, state: FSMContext):
    arg = call.data.split(":")[1]
    if arg == "reset":
        await reset_earned()

    else:
        await UpdateEarned.UE1.set()
        msg_to_edit = await call.message.edit_caption("<b>Напишите значение.</b>",
                                                      reply_markup=back_admin)
        await state.update_data(msg_to_edit=msg_to_edit, arg=arg)
        return
    stat = await select_stat()
    await call.message.edit_caption(f"<b>💰 Ваша прибыль:</b> <code>{stat[3]}.00 Rub</code>",
                                    reply_markup=earned_menu)
...
@dp.message_handler(state=UpdateEarned.UE1)
async def update_earned(message: Message, state: FSMContext):
    data = await state.get_data()
    msg_to_edit, arg = data.get("msg_to_edit"), data.get("arg")
    await message.delete()
    try:
        value = int(message.text)
        await add_earned(value if arg == "add" else -value)
        stat = await select_stat()
        await msg_to_edit.edit_caption(f"<b>Доходность:</b> <code>{stat[3]}</code>",
                                       reply_markup=earned_menu)
        await state.finish()
    except ValueError:
        await msg_to_edit.edit_caption("Не верное значение.\n"
                                       "<b>Напишите значение.</b>")
        ...
        for user in users:
            try:
                await bot.send_message(user[0], text,
                                       parse_mode="HTML", reply_markup=broadcast_menu)
    ...
    if item_type == "product":
        ...
        if message.document:
            if subcategory[5] == "file":
                ...
                try:
                    await msg_to_edit.edit_caption("Товар добавлен, можете закинуть еще.",
