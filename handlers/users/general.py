...
from filters.all_filters import IsNotSubscribed, IsSubscribed
from keyboards.inline.admin_menu import broadcast_menu
...
@dp.message_handler(IsNotSubscribed())
async def bot_start(message: types.Message):
    settings = await select_settings()
    invite_link = await bot.create_chat_invite_link(settings[5])
    await message.answer(f"<b><i>🔐 Доступ запрещён, для получения доступа подпишитесь на новостной канал нашего"
                         f" бота!</i></b>", reply_markup=await invite_menu(invite_link.invite_link))


@dp.callback_query_handler(IsSubscribed(), text="subscribed")
async def subscribed(call: CallbackQuery):
    await call.message.delete()
    await call.message.answer("<b><i>🔓 Доступ разблокирован</i></b>", reply_markup=main_menu_button)
