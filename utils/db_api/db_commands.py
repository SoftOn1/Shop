...
async def reset_earned():
    conn = await aiosqlite.connect('db.db', check_same_thread=False)
    await conn.execute("UPDATE stat SET earned=0")
    await conn.commit()
    await conn.close()


async def add_earned(value):
    conn = await aiosqlite.connect('db.db', check_same_thread=False)
    await conn.execute("UPDATE stat SET earned=earned+?", (value,))
    await conn.commit()
    await conn.close()
...
async def update_settings_req_channel(new):
    conn = await aiosqlite.connect('db.db', check_same_thread=False)
    await conn.execute("UPDATE settings SET req_chat=?", (new,))
    await conn.commit()
    await conn.close()
